# ---- strategy3_resolver.py — Sports Awards/Standings/Drafts Matcher ----
# Category: Sports (events WITHOUT "vs" in title)
...
OUTPUT_PATH = "data/strategy3_pairs.csv"
ERRORS_PATH = "data/strategy3_errors.csv"


# ---- SPORT DETECTION ----

# Detect sport from Polymarket event title
POLY_SPORT_RULES = [
    ("NBA",        [r"\bnba\b", r"basketball", r"\bnba\s"]),
    ("NHL",        [r"\bnhl\b", r"hockey", r"stanley cup"]),
    ("NFL",        [r"\bnfl\b", r"pro football"]),
    ("MLB",        [r"\bmlb\b", r"baseball", r"\bal\s+east\b", r"\bnl\s+east\b",
                    r"\bal\s+west\b", r"\bnl\s+west\b", r"\bal\s+central\b",
                    r"\bnl\s+central\b", r"cy young", r"hank aaron"]),
    ("MLS",        [r"\bmls\b"]),
    ("F1",         [r"\bf1\b", r"formula", r"grand prix"]),
    ("FIFA",       [r"\bfifa\b", r"world cup"]),
    ("EPL",        [r"premier league", r"\bepl\b"]),
    ("La_Liga",    [r"la liga"]),
    ("Serie_A",    [r"serie a"]),
    ("Bundesliga", [r"bundesliga"]),
    ("Ligue_1",    [r"ligue 1"]),
    ("UEFA_CL",    [r"champions league"]),
    ("UEFA_EL",    [r"europa league"]),
    ("UEFA_ECL",   [r"conference league"]),
    ("Tennis",     [r"french open", r"miami open"]),
    ("UFC",        [r"\bufc\b"]),
    ("IPL",        [r"\bipl\b"]),
    ("PGA",        [r"\bpga\b", r"houston open"]),
    ("NCAAM",      [r"\bncaa\b", r"college basketball", r"naismith"]),
    ("FA_Cup",     [r"fa cup"]),
    ("Chess",      [r"fide", r"candidates tournament"]),
    ("Ballon_dOr", [r"ballon d"]),
    ("Cricket",    [r"t20", r"odi", r"cricket"]),
    ("PFA",        [r"pfa"]),
]
...
# Sports that are compatible (can be matched across)
COMPATIBLE_SPORTS = {
    # Each sport is only compatible with itself
}


def detect_poly_sport(title):
    """Detect sport from Polymarket title. Returns sport string or None."""
    t = title.lower()
    for sport, patterns in POLY_SPORT_RULES:
        for pat in patterns:
            if re.search(pat, t):
                return sport
    return None
...
def detect_kalshi_sport(ticker):
    """Detect sport from Kalshi ticker prefix. Returns sport string or None."""
    ticker_upper = ticker.upper()
    for prefix, sport in sorted(KALSHI_SPORT_RULES.items(), key=lambda x: -len(x[0])):
        if ticker_upper.startswith(prefix):
            return sport
    return None


def sports_compatible(poly_sport, kalshi_sport):
    """Check if two detected sports are compatible for matching."""
    # If either is undetectable, allow the match (benefit of doubt)
    if poly_sport is None or kalshi_sport is None:
        return True
    # Must be the same sport
    return poly_sport == kalshi_sport
...
NON_WINNER_PATTERNS = [
    r"^Spread:", r"^Over ", r"^Under ", r"Total ",
    r"More Markets", r"Lower Strikes", r"Toss Match", r"Handicap",
]
...
# ---- ENTITY EXTRACTION + MATCHING (shared with Strategy 1/2) ----
...
STOP_WORDS = {
    ...
    "season", "champion", "championship", "title", "division",
    "conference", "eastern", "western", "award", "trophy",
...
def match_entities(poly_entities, kalshi_entities):
    ...
    for pi, p_entity in enumerate(poly_entities):
        if not p_entity.strip() or is_anonymized(p_entity):
            continue
...
def deduplicate_events(df):
    """Remove More Markets and reverse matchup duplicates at event level."""
    before = len(df)
...
def main():
    print("=" * 60)
    print("STRATEGY 3: Sports Awards/Standings/Drafts Matcher")
    print("=" * 60)

    df = pd.read_csv(CONFIRMED_MATCHES_PATH)
    sports = df[df["category"] == "Sports"].copy()
    non_h2h = sports[~sports["poly_title"].str.contains(" vs", case=False, na=False)].copy()

    print(f"\n  Total sports events: {len(sports)}")
    print(f"  Non-H2H events:     {len(non_h2h)}")

    # Event dedup
    non_h2h = deduplicate_events(non_h2h)

    # Sport-tag verification: reject cross-sport matches
    sport_rejected = 0
    keep_indices = []
    for i, row in non_h2h.iterrows():
        poly_sport = detect_poly_sport(row["poly_title"])
        kalshi_sport = detect_kalshi_sport(row["kalshi_event_ticker"])
        if sports_compatible(poly_sport, kalshi_sport):
            keep_indices.append(i)
        else:
            sport_rejected += 1
            print(f"    ✗ Sport mismatch: Poly={poly_sport} Kalshi={kalshi_sport}")
            print(f"      Poly:   {row['poly_title'][:60]}")
            print(f"      Kalshi: {row['kalshi_title'][:60]} ({row['kalshi_event_ticker']})")

    non_h2h = non_h2h.loc[keep_indices].copy().reset_index(drop=True)
    print(f"  Sport-tag rejected:  {sport_rejected}")
    print(f"  Events to resolve:   {len(non_h2h)}")
    ...
    for i, row in non_h2h.iterrows():
        ...
        for pair in aligned:
            ...
            pair["category"] = "Sports_Awards"
            all_pairs.append(pair)

        if idx % 25 == 0:
            print(f"  [{idx}/{len(non_h2h)}] pairs: {len(all_pairs)} | errors: {len(errors)}")
    ...
    # Report
    print(f"\n{'=' * 60}")
    print(f"STRATEGY 3 RESULTS")
    print(f"{'=' * 60}")
    print(f"  Input non-H2H events:      {len(non_h2h)}")
    ...
    if len(df_pairs) > 0:
        ...
        if len(entity) > 0:
            ...
            print(f"\n  Exact entity matches: {len(same)}")
            print(f"  Non-exact matches:    {len(diff)}")

            if len(diff) > 0:
                print(f"\n  Non-exact matches (ALL):")
                print(f"  {'-' * 55}")
